		...
		<view class="verifyBox">
			...
			<view class="slide-content">
				...
				<view class="slider-pintu">
					<image id="pintuImg" :src="'http://api.herbenspace.cn/files/2023-03-28/' + img + '.jpg'" class="pintu"></image>

					<view class="pintukuai" :style="{ top: top + 'px', left: oldx + 'px' }">
						<image :src="'http://api.herbenspace.cn/files/2023-03-28/' + img + '.jpg'" :style="{ top: '-' + top + 'px', left: '-' + left + 'px'}"></image>
					</view>
				...
				<view class="slider-movearea" @touchend="endTouchMove">
					<movable-area :animation="true"><movable-view :x="x" direction="horizontal" @change="startMove">
						
						<u-icon name="list" color="#fff" size="35"></u-icon>
...
export default {
	...
	data() {
		return {
			x: 0, //初始距离
			oldx: 0, //移动的距离
			img: '5b00a60eb2ca4287b0a0272e664b606d', //显示哪张图片  
			left: 0, //随机拼图的最终X轴距离
			top: 0, //拼图的top距离
		};
	},
	...
	methods: {
		...
		refreshVerify() {
			...
			if (gl <= 0.2) {
				this.img = '5b00a60eb2ca4287b0a0272e664b606d';
			}
			if (gl > 0.2 && gl <= 0.4) {
				this.img = '0158e93de6944f7089dc062e443474a9';
			}
			if (gl > 0.4 && gl <= 0.6) {
				this.img = 'a6ddb2c2169a4f4d93507f571d36a9d6';
			}
			if (gl > 0.6 && gl <= 0.8) {
				this.img = '0158e93de6944f7089dc062e443474a9';
			}
...
.slider-movearea {
	...
	movable-area {
		...
		movable-view {
			...
			// background-image: url('@/static/my/item5.png');
