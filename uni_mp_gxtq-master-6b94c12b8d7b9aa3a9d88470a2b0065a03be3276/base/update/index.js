// #ifdef APP-PLUS
import componentConfig from "@/common/update"
		...
		if (data.updateType == 'forcibly' || data.updateType == 'solicit') {
			...
			dtask = plus.downloader.createDownload(data.downloadUrl, {
				...
				if (status == 200) {
					popupObj.change({
						progressValue: 100,
						progressTip: app.$t("i18n_2"),
						progress: true,
						buttonNum: 0
					});
					plus.runtime.install(download.filename, {}, () => {
						popupObj.change({
							contentText: app.$t("i18n_3"),
							buttonNum: 1,
							progress: false
						});
					}, (e) => {
						popupObj.cancel();
						plus.nativeUI.alert(app.$t('i18n_4') + "[" + e.code + "]：" + e.message);
					});
				} else {
					popupObj.change({
						contentText: app.$t("i18n_5"),
						buttonNum: 1,
						progress: false
					});
				}
					...
					case 3:
						...
						if (progress - lastProgressValue >= 2) {
							...
							popupObj.change({
								...
								progressTip: app.$t("i18n_8") + progress + "%",
								progress: true
							});
						}
						break;
			...
			dtask = plus.downloader.createDownload(data.downloadUrl, {
				...
				if (status == 200) {
					plus.runtime.install(download.filename, {}, () => {
						...
						plus.nativeUI.alert(app.$t('i18n_10') + "[" + e.code + "]：" + e.message);
					});
				} else {
					plus.nativeUI.alert(app.$t("i18n_11"));
				}
			});
			dtask.start();
		}
...
function updatePopup(data, callback) {
	...
	},
	{
		tag: 'font',
		id: 'title',
		text: app.$t("i18n_12") + data.versionName,
		textStyles: {
			size: '18px',
			color: "#333",
			weight: "bold",
...
function downloadPopupDrawing(data) {
	...
	let progressTip = data.progressTip || app.$t("i18n_16");
	let contentText = data.contentText || app.$t("i18n_17");
	let elementList = [
		{
			tag: 'rect', //背景色
			color: '#FFFFFF',
